    ...
    def __init__(self, settings, pretrained_model=None):
        ...
        if self.mode == 'intention':
            ...
            if self.mode == 'addressor_warm' or self.mode == 'addressor':
                ...
                self.memory_past = torch.load('./training/saved_memory/sdd_0.5_15442_filter_past.pt').cuda()
                self.memory_fut = torch.load('./training/saved_memory/sdd_0.5_15442_filter_fut.pt').cuda()
                self.memory_dest = torch.load('./training/saved_memory/sdd_0.5_15442_part_traj.pt').cuda()[:, -1]
