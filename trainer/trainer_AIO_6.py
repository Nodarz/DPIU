...
from models.model_AIO_6 import model_encdec
from trainer.evaluations_6 import *
...
class Trainer:
    def __init__(self, config):
        ...
        self.folder_test = 'training/training_' + config.mode + '/' + self.name_test + '_' + config.info + '_6'
    ...
    def fit(self):
        ...
        print('final value:{}'.format(minValue))
        if self.config.mode == 'intention':
            # Generate memory banks.
            # model_pretrain_intention = torch.load(self.folder_test + 'model_ae_' + self.name_test)
            model_pretrain_intention = torch.load('./training/training_intention/2023-04-21_try1_6/model_ae_2023-04-21')
            self.mem_n2n.load_state_dict(model_pretrain_intention.state_dict())
            self.mem_n2n.generate_memory(self.train_dataset, filter_memory=True, threshold_distance=0.375)
